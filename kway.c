...
void cria_matriz(char **matriz, unsigned long int tamanho_matriz) //Cria a matriz
{
    matriz = (char**)malloc(tamanho_matriz * sizeof(char*)); //Aloca memória
...
unsigned long int criarArquivosOrdenados(char *nome_arquivo_entrada, unsigned long int tam_ram) 
{
    ...
    printf("Arquivos sendo Criado, pode demorar um pouco\n");
...
{
    int pos = -1;
    for(int i = 0; i < numArqs; i++) //Para percorrer os arquivos
    {
        ...
        //Se o buffer ainda nao foi todo percorrido
        if(lista_buffers[i]->pos_atual_matriz < lista_buffers[i]->pos_max_matriz)
        {
            ...
            else
            {
                ...
                //Cria a matriz de string, com base no buffer lido
    ...
    if(pos != -1)
    {
        ...
        if(lista_buffers[pos]->pos_atual_matriz == lista_buffers[pos]->pos_max_matriz)
        {
            ...
            if(lista_buffers[pos]->posicao < lista_buffers[pos]->fim_arquivo)
            {
                ...
                carrega_Buffer(lista_buffers[pos]);
...
void merge(char *nome_arq_saida, int numArqs, unsigned long int pedaco_ram)
{
    ...
    for(int i = 0; i < numArqs; i++)
    {
        ...
        carrega_Buffer(lista_buffers[i]);
    ...
    printf("Fazendo a comparação (Merge nos arquivos)\n");
    while(procuraMenor(lista_buffers, numArqs, menor))
    {
        ...
        else
        {
            ...
            escreve_item(nome_arq_saida, buffer);
...
void delete_temp_files(int numArqs)
{
    ...
    printf("Apagando os arquivos temporários\n")  ;
    for(int i = 0; i< numArqs; i++)
    {
...
//Função que faz as comparações - K-WAY
void k_way_merge(char *nome_arquivo_entrada, char *nome_arq_saida, unsigned long int tam_ram)
{
    ...
    printf("--> Qtd Arquivos: %ld\n", numArqs);
    
    
    unsigned long int pedaco_ram = tam_ram / (numArqs + 1);

    printf("--> Qtd de RAM utilizada em cada arquivo: %ld Bytes\n", pedaco_ram);
    
    if(pedaco_ram > 200)
    {
        //Cria o arquivo de saida ordenado
        merge(nome_arq_saida, numArqs, pedaco_ram);
        printf("--> O arquivo foi ordenado\n--> Saida: %s\n", nome_arq_saida);
    }
    else
    {
        printf("--> RAM insuficiente.\n");
    }
