...
{
    char **matriz = (char**)malloc(tamanho_matriz * sizeof(char*)); //Alocando memória
...
//Calcula a metade do arquivo
void half_file(char *nome_arquivo, char* half_arquivo_1, char *half_arquivo_2)
{
    unsigned long int tamanho_arquivo = tam_Arquivo(nome_arquivo);
    ...
    carrega_Buffer(buffer_original_file);
    escreve_item(half_arquivo_1, buffer_original_file);

    carrega_Buffer(buffer_original_file);
    escreve_item(half_arquivo_2, buffer_original_file);

    //Divisao ao meio nao seja exata
...
//Deleta arquivo vazio
void delete_empty_file(char *nome_arquivo)
{
    unsigned long int has_content = tam_Arquivo(nome_arquivo);
    if(!has_content)
    {
        remove(nome_arquivo);;
    }
...
//Quantidade de linhas dobra a cada RODADA
void run_merging(unsigned long int qtd_linhas, char *origem_1, char *origem_2, char *dest_1, char *dest_2)
{
    ...
    //Verificando se os arquivos são válidos
    ...
    {   
        ...
        limpa_arquivo(origem_1);
        limpa_arquivo(origem_2);
...
int main(int argc, char *argv[])
{
    if(argc < 3)
    {
        printf("ERROR: Favor informar o arquivo de entrada e inicio da Rodada!\n");
        printf(">>> EXEMPLO: execm Arquivo_entrada 1\n");
        return 0;
    }
    time_t inicio, fim, total;
    inicio = time(NULL);

    //Normalmente inicado em 1, mas pode mudar
    ...
    limpa_arquivo(aux_arq_1);
    limpa_arquivo(aux_arq_2);
    limpa_arquivo(aux_arq_3);
    limpa_arquivo(aux_arq_4);
    
    
    //Divido o arquivo original ao meio
    printf(">>>Dividindo arquivo em dois<<<\n\n");
